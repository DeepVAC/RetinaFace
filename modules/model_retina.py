...
class RetinaFaceRepVGGBackbone(RepVGGASmall):
    def __init__(self, class_num=1000, deploy=False):
        super(RetinaFaceRepVGGBackbone, self).__init__(class_num, deploy)
        self.return_layers = [5, 19, 20]

    def forward(self, x):
        out = []
        x = self.stage0(x)
        for i, cur in enumerate(self.layer):
            x = cur(x)
            if i in self.return_layers:
                out.append(x)
...
class RetinaFaceRepVGG(RetinaFaceMobileNet):
    ...
    def auditConfig(self):
        ...
        self.out_channels = 64
        self.body = RetinaFaceRepVGGBackbone(self.class_num, self.deploy)
